...
export class StaffService {
  ...
  async create(createStaffDto: CreateStaffDto): Promise<string> {
    ...
    if (createStaffDto.role === Role.VENUE_MANAGER) {
      if (!createStaffDto.venueManagerAssignments || createStaffDto.venueManagerAssignments.length === 0) {
        throw new BadRequestException('Venue manager role requires at least one venue assignment');
      }
    }

    if (createStaffDto.role === Role.DUTY_MANAGER) {
      if (!createStaffDto.dutyManagerAssignments || createStaffDto.dutyManagerAssignments.length === 0) {
        throw new BadRequestException('Duty manager role requires at least one venue assignment');
      }
    }
    ...
    const newStaff = await this.prisma.$transaction(async (tx) => {
      ...
      if (createStaffDto.role === Role.VENUE_MANAGER && createStaffDto.venueManagerAssignments) {
      ...
      if (createStaffDto.role === Role.DUTY_MANAGER && createStaffDto.dutyManagerAssignments) {
